...
async function loadData() {
    ...
    if (data.stats) {
        ...
        if (el) {
            el.textContent = isNaN(todayCount) ? 0 : todayCount;
        }
    ...
    if (data.seenJobs) {
        ...
        if (totalEl) {
            totalEl.textContent = Array.isArray(data.seenJobs) ? data.seenJobs.length : 0;
        }
    }

    const tracked = trackedData.trackedProjects || {};
    const trackedList = Object.values(tracked).sort((a, b) =>
    ...
    if (proposalEl) {
        proposalEl.value = data.proposalTemplate || '';
    }
