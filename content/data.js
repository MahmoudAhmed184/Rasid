...
function extractProjectData() {
    ...
    const metaRows = document.querySelectorAll(
        '.meta-row, .table-meta tr, .card .table tr, li.meta-item'
    );
    metaRows.forEach((row) => {
        const label =
            row
                .querySelector('.meta-label, td:first-child, .meta-item-label')
                ?.textContent.trim()
                .replace(/\s+/g, ' ') ||
            row.innerText.split(/[:\n]/)[0]?.trim().replace(/\s+/g, ' ');
        const value =
            row
                .querySelector('.meta-value, td:last-child, .meta-item-value')
                ?.textContent.trim()
                .replace(/\s+/g, ' ') ||
    ...
    const budgetEl = document.querySelector(
        '[data-type="project-budget_range"], #project-meta-panel .meta-value[data-type="project-budget_range"]'
    );
    if (budgetEl) {
    ...
    const timeEl = document.querySelector(
        'time[itemprop="datePublished"], #project-meta-panel time'
    );
    if (timeEl) {
    ...
    const tags = Array.from(
        document.querySelectorAll('.skills .tag, .tags .tag, .project-tags .tag')
    )
        .map((tag) => tag.textContent.trim())
        .join(', ');

    const titleEl =
        document.querySelector('.heada__title span[data-type="page-header-title"]') ||
    ...
    return {
        ...
        attachments: Array.from(
            document.querySelectorAll('#projectDetailsTab #project-files-panel .attachment a[href]')
        ).map((a) => ({
            url: a.href,
...
function getProjectDescription() {
    ...
    const container =
        document.querySelector('#projectDetailsTab') || document.querySelector('#project-brief');
    if (!container) {
        return '';
    }

    const mainText = container.querySelector(
        '.carda__content, .text-wrapper-div:not(.field-label)'
    );
    if (mainText) {
...
async function fetchDeepProjectData(url) {
    try {
        ...
        res.title =
            doc
                .querySelector('.heada__title span[data-type="page-header-title"]')
                ?.innerText.trim() ||
            doc.querySelector('.page-title h1')?.innerText.trim() ||
            doc.title.split('-')[0].trim();

        res.category =
            doc.querySelector('.breadcrumb li:nth-last-child(2) a')?.innerText.trim() ||
            doc.querySelector('.project-header__meta a')?.innerText.trim();

        res.status =
            getMetaValue('حالة المشروع') ||
            doc.querySelector('.project-header .label')?.innerText.trim().replace(/\s+/g, ' ');
        ...
        if (clientCard) {
            ...
            res.clientName =
                clientCard.querySelector('.profile__name')?.innerText.trim() ||
        ...
        const container =
            doc.querySelector('#projectDetailsTab') || doc.querySelector('#project-brief');
        let fullDesc = '';
        if (container) {
            const mainText = container.querySelector(
                '.carda__content, .text-wrapper-div:not(.field-label)'
            ...
            detailRows.forEach((row) => {
                const label = row.querySelector('.field-label')?.textContent.trim();
                const value = row
                    .querySelector('.text-wrapper-div:not(.field-label)')
                    ?.textContent.trim();
                if (label && value && label !== value) {
        ...
        res.attachments = Array.from(
            doc.querySelectorAll('#projectDetailsTab #project-files-panel .attachment a[href]')
        ).map((a) => ({
            url: a.href,
            name: a.getAttribute('title') || a.innerText.trim(),
        }));
...
function extractMyProposalFull(externalProjectData = null) {
    try {
        ...
        const contentEl =
            targetProposal.querySelector('.bid__details .text-wrapper-div') ||
        ...
        const data = {
            ...
            attachments: Array.from(
                targetProposal.querySelectorAll('#bid-attachments .attachment a[href]')
            ).map((a) => ({
                url: a.href,
                name: a.getAttribute('title') || a.innerText.trim(),
            })),
