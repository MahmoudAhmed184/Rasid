...
function absoluteNafezlyUrl(href: string | null): string {
    if (!href) {
        return '';
    }

    return href.startsWith('http') ? href : `https://nafezly.com${href}`;
}
...
export function parseNafezlyProjectHtml(html: string): Partial<JobRecord> | null {
    ...
    const tags = [...doc.querySelectorAll<HTMLAnchorElement>('a.tag-class[href*="/projects/skill"]')]
